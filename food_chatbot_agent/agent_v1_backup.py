...
def search_restaurants_by_cuisine(cuisine: str) -> str:
    """Search restaurants by cuisine type - Note: Cuisine filtering not available, returns all restaurants"""
    try:
        response = requests.get(f"{FASTAPI_BASE_URL}/restaurants/")
        if response.status_code == 200:
            ...
            if not all_restaurants:
                return f"❌ No restaurants found. Would you like to try something else?"
            
            # Since cuisine field is not available, return all restaurants with a note
            result = f"I found {len(all_restaurants)} restaurant(s) (Note: Cuisine-based filtering is not available):\n\n"
